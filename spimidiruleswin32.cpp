...
//
//2014dec05, creation of spimidiruleswin32.cpp 
//
//2014dec05, showbytes() has not been revised, replace putchar()
//
//xznn.com, xznnworldwide@gmail.com, carl w.b. poirier
//nakedsoftware.org, spi@oifii.org or stephane.poirier@oifii.org
////////////////////////////////////////////////////////////////
...
void midiOutAllNotesOff(const char* mididevicename, int channelid);
...
TCHAR szTitle[1024]={L"spimidiruleswin32title"};					// The title bar text
TCHAR szWindowClass[1024]={L"spimidiruleswin32class"};			// the main window class name
        ...
        if (count == 1) 
		{
			...
			if(chan==global_inputmidichannel)
			{
				...
				if(command==MIDI_CTRL)
				{
					...
					if(data1==41)
					{
						//ShellExecuteA(NULL, "open", ".\\spimidinote.exe", "-b \"\" -e \"\" -d \"Out To MIDI Yoke:  1\" 0 60 5000 100", NULL, false);
						//midiOutNoteOnNoteOff("Out To MIDI Yoke:  1", 0, 60, 5000, 127);
						midiOutNoteOnNoteOff("Out To MIDI Yoke:  1", 0, Note21(), 5000, 127);
					}
				}
				else if(command==MIDI_ON_NOTE)
				{
					//ShellExecuteA(NULL, "open", ".\\spimidicontinuouscontroller.exe", "-b \"\" -e \"\" -d \"Out To MIDI Yoke:  2\" 0 64 1 101 5 700 300000", NULL, false);
					//midiOutContinuousController("Out To MIDI Yoke:  1", 0, 64, 1, 101, 5, 700, 30000);
					midiOutNoteOnNoteOff("Out To MIDI Yoke:  1", 0, Note21(), 5000, 127);
				}
...
void CALLBACK StartGlobalProcess(UINT uTimerID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2)
{
    ...
    swprintf(pWCHAR, L"spimidiruleswin32 ready.\n");StatusAddText(pWCHAR);
    global_active = true;
...
void midiOutAllNotesOff(const char* mididevicename, int channelid)
{
	...
	{
		midiOutNoteOff(mididevicename, channelid, notenumber);
	}
}
...
void midinote(const char* mididevicename, int channelid, int notenumber, int duration, int velocity) 
{
	...
	*/
	char szparameters[128];
	sprintf(szparameters, "-b \"\" -e \"\" -d \"%s\" %d %d %d %d", mididevicename, channelid, notenumber, duration, velocity);

	//ShellExecuteA(NULL, "open", ".\\spimidinote.exe", "-b \"\" -e \"\" -d \"Out To MIDI Yoke:  1\" 0 60 5000 100", NULL, false);
	ShellExecuteA(NULL, "open", ".\\spimidinote.exe", szparameters, NULL, false);
}
...
void midiprogramchange(const char* mididevicename, int channelid, int programnumber) 
{
	...
	*/
	...
	char szparameters[128];
	sprintf(szparameters, "-b \"\" -e \"\" -d \"%s\" %d %d", mididevicename, channelid, programnumber);

	//ShellExecuteA(NULL, "open", ".\\spimidiprogramchange.exe", "-b \"\" -e \"\" -d \"Out To MIDI Yoke:  5\" 0 0", NULL, false);
	ShellExecuteA(NULL, "open", ".\\spimidiprogramchange.exe", szparameters, NULL, false);
}
...
void midicontinuouscontroller(const char* mididevicename, int channelid, int ccnumber, int ccstart, int ccend, int ccstep, int startdelay_ms, int serieduration_ms) 
{
	...
	*/
	char szparameters[128];
	sprintf(szparameters, "-b \"\" -e \"\" -d \"%s\" %d %d %d %d %d %d %d", mididevicename, channelid, ccnumber, ccstart, ccend, ccstep, startdelay_ms, serieduration_ms);

	//ShellExecuteA(NULL, "open", ".\\spimidicontinuouscontroller.exe", "-b \"\" -e \"\" -d \"Out To MIDI Yoke:  2\" 0 64 1 101 5 700 300000", NULL, false);
	ShellExecuteA(NULL, "open", ".\\spimidicontinuouscontroller.exe", szparameters, NULL, false);
}
...
int Note21()
{
	...
	//std::wstring szDegreesFromTonic = RegReadSZ(HKEY_CURRENT_USER,L"Software\\XZNN\\ESNSEalfa\\FOM\\Note",L"ScalePattern");
	//std::wstring szTonality = RegReadSZ(HKEY_CURRENT_USER,L"Software\\XZNN\\ESNSEalfa\\FOM\\Note",L"Tonality");
	//std::wstring szOctave = RegReadSZ(HKEY_CURRENT_USER,L"Software\\XZNN\\ESNSEalfa\\FOM\\Note",L"Octave");
	std::wstring szDegreesFromTonic=L"A";
	std::wstring szTonality=L"0";
	std::wstring szOctave=L"3";

	int DegreesFromTonic = 0;
	int Tonality = atoi(utf8_encode(szTonality).c_str());
	int Octave = atoi(utf8_encode(szOctave).c_str());

	if(szDegreesFromTonic.c_str()==L"A")
	{
		DegreesFromTonic = 0;
	}
	else if(szDegreesFromTonic.c_str()==L"B")
	{
		DegreesFromTonic = -2;
	}
	else if(szDegreesFromTonic.c_str()==L"C")
	{
		DegreesFromTonic = -4;
	}
	else if(szDegreesFromTonic.c_str()==L"D")
	{
		DegreesFromTonic = 0;
	}
	else if(szDegreesFromTonic.c_str()==L"E")
	{
		DegreesFromTonic = -2;
	}
	else if(szDegreesFromTonic.c_str()==L"F")
	{
		DegreesFromTonic = -3;
	}
	else if(szDegreesFromTonic.c_str()==L"G")
	{
		DegreesFromTonic = -2;
	}


	return DegreesFromTonic + Tonality + (12 * Octave);
}
